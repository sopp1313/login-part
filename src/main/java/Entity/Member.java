...
public class Member {
    ...
    private static final Pattern idd =
            Pattern.compile("^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])[a-zA-Z0-9]*$");
    private static final Pattern passwordd =
            Pattern.compile("^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])[a-zA-Z0-9]*$");

    @Id
    private String logInId;   // PK: 수동 할당(String). @GeneratedValue 제거
    private String password;  // 해시를 저장
    private String name;
